...
/* 
 * These next two functions are for 
 * initializing and displaying the 
 * naming table in the DNS server
 */

/* Initialize the naming table to be empty w/ no valid domain names */
...
struct naming_table naming_table; /* DNS Servers naming table */

file_buf_init(&f_buf_upload);
file_buf_init(&f_buf_download);


/* Initialize the DNS_Servers naming table*/
init_dns_table(&naming_table);

/* Debug: Print the initialized DNS table (should see that its empty) */
print_dns_table(&naming_table);
